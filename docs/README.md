...
# stochLAB <img src='images/logo.png' align="right" height="139" />
...
### Relationship between the sCRM and Band(2012)

The stochastic collision risk model is simply a wrapper around the Band (2012)
collision risk model. Every iteration of the sCRM is a single run of `band_crm()`, 
where all the input parameters are sampled based on a mean and standard deviation.  

The most important difference is in how flight height distributions are treated
between both functions. For the `band_crm()` function, a single flight height 
distribution is used (guidance is to use the maximum likelihood estimated distribution
currently in Johnston et al 2014). For `stoch_crm()`, the flight height distributions are
sampled from a series of bootstrap iterations, with the final result therefore
tending towards a mean flight height scenario which is not likely to be the same
as the maximum likelihood scenario. This can be demonstrated by removing the 
stochastic element from `stoch_crm()` and using the same flight height distribution
in both functions. 
...
# Select a SINGLE flight height bootstrap (i.e. one FH distribution)
# This scenario is for the LEFT-hand panel in the below figure.
fhd_boots <- generic_fhd_bootstraps[[1]][,1:2]


# Select ALL flight height bootstraps
# This scenario is for the RIGHT-hand panel in the below figure.
fhd_boots <- generic_fhd_bootstraps[[1]]
...
stoch_output <- stoch_crm(
  ...
  prop_crh_pars = data.frame(mean = 0.13, sd = 0),
  bird_dens_opt = "tnorm",
  bird_dens_dt = b_dens,
  flight_type = "flapping",
  prop_upwind = 0.5,
...
<img src='images/Band_v_stoch.png' align="center" height="400" />


In the left-hand panel, you can see how the outputs from `stoch_crm()` and 
`band_crm()` are the same when we use a single FH distribution.
In the right-hand panel, the estimates are slightly different when we use all the flight 
height distributions in the `stoch_crm()` function. Note that option 1 does not
change in both cases because it does not depend on the flight height distribution and
shows that all input parameters are the same, thus producing the same estimates.
