...
c_densOpt <- "reSamp"
  ...
  for (s in 1 : length (CRSpecies)){
    ...
    ## Subsets the species bio data (avoidance rates, etc...)
    species.dat = subset (BirdData, Species == CRSpecies[s])
    ## Sets the value to 1 if the bird has a flapping flight
    ...
    ### The user will select how the density data are to be represented.
    ### truncNorm means that the input count data 
    ...
    flight.boot.sample <- sample(flight.boot, iter, replace=T)  ## Proportion of birds in 1m bands bootstrapped 200 times
