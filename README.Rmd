...
In addition, previous code underpinning key calculations for the extended model
...
particularly beneficial under a stochastic context, when Band calculations are
computed repeatedly.
...
seabird species and one turbine/windfarm scenario, with fictional input
parameter data.
...
1. Suggest how input parameter data sets used in the previous implementation can
be reshaped to fit `stoch_crm()`'s  interface. Suggested code is also relevant
in the context of multiple scenarios applications, as the wide tabular format of
these data sets is likely preferred structure for users to gather input
parameters under different scenarios.

2. Set up the groundwork to run stoch_crm() for multiple species and
